#Code adapted from https://github.com/sidhujyatha/ANCOM and modified by Gloria Fackelmann (gloria.fackelmann@uni-ulm.de; https://www.researchgate.net/profile/Gloria-Fackelmann) to always run triangulation method instead of parallel cores and to show more W_stat cutoff values (0.60-0.95).
...
ancom.W = function(otu_data,var_data,
  ...
  if(FALSE){
    ...
    aa <- bb <- NULL
    logratio.mat <- foreach(bb = 1:n_otu, .combine = "cbind", 
      ...
      if(aa >= bb){
        ...
        data.pair <- data_comp[,which(colnames(data_comp)%in%otu_ids[c(aa,bb)])]
        ...
        if(adjusted==FALSE&repeated==FALSE){  ## Wilcox, Kruskal Wallis
          ...
          p_out <- summary(model)[[1]][["Pr(>F)"]][picker]
          ...
          p_out <- anova(model)[["p-value"]][picker]
        }
      }
